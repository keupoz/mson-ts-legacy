...
export class JsonBox extends JsonComponent {
  ...
  constructor (context: JsonContext, _name: string, json: JsonObject) {
    ...
    this.from = context.getLocals().getMemberArray(json, 'from', 3)
    this.size = context.getLocals().getMemberArray(json, 'size', 3)
